...
# from werkzeug import secure_filenamec
import imutils
...
def upload_image():
    if request.method == "POST":
        if request.files:
            ...
            ANSWER_KEY = Convert(json.loads(response.text))
            print(ANSWER_KEY)
            ...
            for (q, i) in enumerate(np.arange(0, len(ANSWER_KEY)*4, 4)):
            ...
            # print(base64.b64encode(paper))
            # print(paper)
            # print(paper.tobytes())

            # pic = cv2.imencode('.jeg',paper)
            # pic2 = np.getbuffer(pic, dtype=np.uint8)
        ...
        return {"msg":"Ok","score":score,"image":final,"key":ANSWER_KEY}
